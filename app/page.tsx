...
import { getAbout, getCTA, getCampaigns, getFaqs, getFinalCTA, getFlow, getHero, getLessons, getNavItems, getPrices, getReasons, getReviews, getServices, getSessionImages, getStores, getWorries } from "@/app/lib/notion";

export const dynamic = "force-dynamic";
export const revalidate = 0;

const campaignDeadline = "8月31日まで";
const campaignLimitCount = "20名様";
...
const getServiceSupportText = (name: string) => {
  if (name.includes("姿勢")) return "鏡を見るたび、すっと整った印象へ。";
  if (name.includes("反り腰") || name.includes("猫背")) return "身体のクセに合わせて、無理なくリセット。";
  if (name.includes("肩") || name.includes("腰痛")) return "重だるさを手放して、軽やかに動ける毎日へ。";
  if (name.includes("ダイエット") || name.includes("ボディ")) return "しなやかに引き締める、女性らしいラインづくり。";
  if (name.includes("習慣")) return "週1回から、自分を整える時間を習慣に。";
...
const getServiceImage = (name: string) => {
  ...
  if (name.includes("反り腰") || name.includes("猫背")) {
    return {
      alt: "ピラティスマシンで背中のストレッチをする女性",
      objectPosition: "center 48%",
      src: "/images/services/back-posture.jpg",
    };
  }

  if (name.includes("肩") || name.includes("腰痛")) {
    return {
      alt: "肩まわりと背中を伸ばす女性",
      objectPosition: "center 38%",
      src: "/images/services/shoulder-back.jpg",
    };
  }

  if (name.includes("ボディ") || name.includes("ダイエット")) {
    return {
      alt: "ピラティスマシンで脚と体幹を鍛える女性",
      objectPosition: "center 45%",
      src: "/images/services/bodymake.jpg",
    };
  }

  if (name.includes("習慣")) {
    return {
      alt: "利用者とインストラクターが笑顔で会話する様子",
      objectPosition: "center 35%",
      src: "/images/services/habit.jpg",
    };
  }
...
const heroCampaign = campaigns[0] ?? {
  ...
  description: "",
  label: "今月限定",
  period: campaignDeadline,
  limitedText: `${campaignLimitCount}限定`,
  ...
  <div className="mx-auto max-w-6xl">
    <div className="mx-auto mb-8 max-w-md rounded-[28px] border border-[#D8EAC7] bg-white px-5 py-9 text-center shadow-[0_16px_45px_rgba(23,35,59,0.08)] md:mb-12 md:max-w-2xl md:px-10 md:py-12">
      <p className="text-[19px] font-extrabold leading-snug tracking-normal text-[#545454] md:text-[28px]">
        <span className="block md:inline">スタジオONCE</span>
        <span className="block md:inline">江戸川橋・護国寺店</span>
      </p>

      <p className="mt-3 text-[24px] font-extrabold leading-tight tracking-normal text-[#E89A3D] md:text-[40px]">
        <span className="block md:inline">オープニングキャンペーン</span>
        <span className="block md:inline">実施中✨</span>
      </p>

      <p className="mt-5 text-[13px] font-extrabold leading-relaxed text-[#545454] md:text-base">
        <span className="block md:inline">江戸川橋駅 徒歩5分</span>
        <span className="hidden md:inline"> ｜ </span>
        <span className="block md:inline">護国寺駅 徒歩7分</span>
      </p>
    </div>
          ...
          <div className="grid gap-8 md:grid-cols-[1fr_1fr] md:items-center">
            ...
            <div className="rounded-[28px] bg-[#DDE8CC]/45 p-6 text-center md:p-8">
              ...
              <p className="mx-auto mb-5 inline-flex rounded-full bg-white px-5 py-2 text-sm font-bold text-[#545454] shadow-sm">
                <span>{campaignDeadline}先着</span>
                <span className="!text-[#E89B3A]">{campaignLimitCount}</span>
                <span>限定</span>
              </p>
  ...
  >
    ✓ {item}
      ...
      <section className="bg-white px-6 py-16 text-center md:py-20">
        <div className="mx-auto max-w-3xl rounded-[36px] border border-[#D8EAC7]/70 bg-[#F8FBF4] px-6 py-12 shadow-[0_22px_60px_rgba(108,143,93,0.12)] md:px-12 md:py-14">
          <div className="mx-auto mb-5 h-px w-24 bg-[#E89A3D]/50" />

          <div className="mb-5 text-[46px] font-light leading-none text-[#E89A3D] md:text-[50px]" aria-hidden="true">
            ↓
          </div>

          <h2 className="text-[30px] font-bold leading-[1.18] text-[#545454] md:text-[42px]">
            スタジオONCEなら
            <br />
            そのお悩み、
            <br />
            <span className="inline-block pt-1 text-[34px] text-[#E89A3D] md:text-5xl">解決できます！</span>
          </h2>

          <div className="mx-auto mt-7 h-1 w-20 rounded-full bg-[#D8EAC7]" />
        </div>
      </section>

      {/* 5 ONCEでできること */}
      <section id="services" className="scroll-mt-24 bg-[#FFFDF8] px-6 py-20">
        <div className="mx-auto mb-8 max-w-3xl text-center">
          ...
          <p className="mx-auto mt-4 max-w-2xl text-sm font-semibold leading-7 text-[#6C6C6C] md:text-base">
            姿勢も、身体の軽さも、見た目の印象も。
            <br className="hidden md:block" />
            あなたの身体に合わせたマンツーマンで、通うたびに前向きな変化を。
          </p>
        </div>
        ...
        <div className="mx-auto grid max-w-5xl gap-4 sm:grid-cols-2 md:grid-cols-3 md:gap-5">
          {(services.length > 0
            ...
            const hasServiceImage = serviceImage ? localImageExists(serviceImage.src) : false;

            return (
            <div
              key={item}
              className="group relative flex h-full flex-col overflow-hidden rounded-[28px] border border-[#D8EAC7]/80 bg-white p-5 shadow-[0_18px_45px_rgba(108,143,93,0.12)] transition hover:-translate-y-0.5 hover:shadow-[0_24px_55px_rgba(108,143,93,0.18)] md:p-6"
            >
              <div className="pointer-events-none absolute -right-10 -top-12 h-28 w-28 rounded-full bg-[#E89A3D]/12" />
              <div className="pointer-events-none absolute -bottom-12 left-6 h-24 w-24 rounded-full bg-[#D8EAC7]/45" />

              <div className="relative mb-5 h-40 overflow-hidden rounded-[22px] bg-[#F3F8ED] md:h-44">
                {serviceImage && hasServiceImage ? (
                  <Image
                    ...
                    style={{ objectPosition: serviceImage.objectPosition }}
                  />
                ) : (
                  <div className="flex h-full items-center justify-center bg-[linear-gradient(135deg,#F8FBF4_0%,#EAF4E0_100%)] px-6 text-center text-xs font-bold leading-6 tracking-[0.18em] text-[#6C8F5D]">
                    IMAGE
                    <br />
                    COMING SOON
                  </div>
              ...
              <div className="relative mb-5 flex items-center justify-between gap-4">
                <div className="flex h-12 w-12 items-center justify-center rounded-full bg-[#D8EAC7] text-sm font-extrabold text-[#545454] shadow-sm">
                  {String(index + 1).padStart(2, "0")}
                </div>

                <span className="rounded-full bg-[#E89A3D]/10 px-3 py-1 text-[11px] font-bold tracking-[0.16em] text-[#E89A3D]">
                  PERSONAL
                </span>
              </div>

              <p className="relative text-xl font-bold leading-snug text-[#545454] md:text-[22px]">
                {item}
              </p>

              <p className="relative mt-3 text-sm font-semibold leading-7 text-[#6C6C6C]">
                {getServiceSupportText(item)}
              </p>

              <div className="relative mt-5 h-1 w-16 rounded-full bg-[#E89A3D]/70 transition group-hover:w-24" />
            </div>
          );
          })}
        </div>
            ...
            : [
                {
                  label: "今月限定",
                  period: campaignDeadline,
                  limit: `先着${campaignLimitCount}限定`,
                  title: "キャンペーン",
